const API = {
  ...
  profile: {
    get: () => API.request('GET', '/profile'),
    updatePassword: (data) => API.request('PUT', '/profile/password', data),
    updateEmail: (email) => API.request('PUT', '/profile/email', { email }),
    updateDisplayName: (name) => API.request('PUT', '/profile/display-name', { displayName: name }),
    uploadAvatar: (data) => API.request('POST', '/profile/avatar', { avatar: data }),
    removeAvatar: () => API.request('DELETE', '/profile/avatar'),
    setup2FA: () => API.request('POST', '/profile/2fa/setup'),
    verify2FA: (token) => API.request('POST', '/profile/2fa/verify', { token }),
    disable2FA: (data) => API.request('POST', '/profile/2fa/disable', data),
    sessions: () => API.request('GET', '/profile/sessions'),
    revokeSession: (id) => API.request('DELETE', '/profile/sessions/' + id),
    activity: (limit) => API.request('GET', '/profile/activity' + (limit ? '?limit=' + limit : '')),
  },
  notifications: {
