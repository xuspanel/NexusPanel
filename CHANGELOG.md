...
- **Databases Screen SASL error** — the panel showed a cryptic `SASL: SCRAM-SERVER-FIRST-MESSAGE: client password must be a string` when you opened the Databases screen because PostgreSQL was installed but never provisioned: the installer installed the `postgresql` package but never set a password, never configured loopback TCP auth, and never wrote `DB_PASSWORD` (or `DB_HOST`/`DB_PORT`/`DB_USER`) into the panel's `.env` file. The `pg` driver then tried SCRAM auth with an empty/missing password, crashed inside the SASL flow, and the entire Databases view showed a raw stack trace instead of a helpful message. Now:
  - A new shared `provision_postgres()` function (in `install-common.sh`, called by every OS installer when `--postgres` is used) generates a strong random password for the `postgres` superuser, forces `scram-sha-256` for local TCP (replacing `ident`/`trust`/`peer` on RHEL/Debian/macOS Homebrew), reloads the server, and verifies the login works — the password is then written into `.env` as `DB_PASSWORD`. Covers Ubuntu, Debian, AlmaLinux, Rocky, CentOS, Fedora, and macOS.
  - The `.env` template now always writes `DB_HOST`/`DB_PORT`/`DB_USER`/`DB_PASSWORD` with sensible defaults (`127.0.0.1`, `5432`, `postgres`), and on reinstall it preserves any existing values the admin may have customised.
  - The `src/services/databases.js` pool driver now coerces the password to a string with a nullish-coalescing fallback, and every database query is wrapped in a friendly error mapper: SASL auth errors are surfaced as “PostgreSQL login failed: the panel has no usable DB_PASSWORD configured — run the installer with `--postgres`”, `ECONNREFUSED` says to start the server, wrong-credentials says which user/target failed, and “role does not exist” tells the admin to fix `DB_USER`. The Databases screen no longer shows raw pg driver internals.

### Changed
- macOS installer: `parse_args` is now called in `main()` so `--postgres` and `--docker` flags actually take effect on macOS (they were silently ignored before). PostgreSQL is installed via Homebrew and provisioned the same way as on Linux.

## [1.35.5] - 2026-08-07
### Fixed
- Large file uploads could stall forever at `0%` in the File Manager: nginx generated by the installer had no `client_max_body_size`, so uploads over nginx's 1 MB default were rejected with an HTML `413` page, and the upload promise's `JSON.parse(xhr.responseText)` threw on that non-JSON response inside the event listener — the promise never settled and the progress bar froze (e.g. `0% 80 KB / 50.3 MB`). The installer now writes `client_max_body_size 500m` into both the HTTP and HTTPS vhosts (matching the panel's 500 MB limit), and the upload handler now parses responses safely and surfaces clear status-aware errors: a `413` shows an actionable toast telling you to raise `client_max_body_size` in nginx, and the progress UI is cleared instead of freezing

### Changed
- `filemanager.js` cache-buster bumped to `v=1.35.5` so the upload fix reaches browsers without a manual cache clear
