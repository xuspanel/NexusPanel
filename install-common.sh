...
generate_env_file() {
  ...
  # Preserve existing DB credentials on reinstall/upgrade, otherwise use
  # the values generated by provision_postgres (if --postgres was requested)
  # or the panel defaults.
  local existing_db_host existing_db_port existing_db_user existing_db_pass
  existing_db_host="$(grep -E '^DB_HOST=' "${env_file}" 2>/dev/null | head -1 | cut -d= -f2- || true)"
  existing_db_port="$(grep -E '^DB_PORT=' "${env_file}" 2>/dev/null | head -1 | cut -d= -f2- || true)"
  existing_db_user="$(grep -E '^DB_USER=' "${env_file}" 2>/dev/null | head -1 | cut -d= -f2- || true)"
  existing_db_pass="$(grep -E '^DB_PASSWORD=' "${env_file}" 2>/dev/null | head -1 | cut -d= -f2- || true)"
...
# --- PostgreSQL (Databases screen / pgAdmin) ---
# When --postgres is used the installer provisions a local server and writes
# a generated password.  These values are preserved across reinstalls.
DB_HOST=${existing_db_host:-127.0.0.1}
DB_PORT=${existing_db_port:-5432}
DB_USER=${existing_db_user:-postgres}
DB_PASSWORD=${existing_db_pass:-${PG_PASSWORD:-}}
...
# Called by OS installers after installing PostgreSQL (--postgres). It:
#   1. waits for the server to accept connections,
#   2. ensures a "postgres" superuser exists and generates a strong password,
#   3. forces SCRAM auth for local TCP so the panel can log in over 127.0.0.1,
#   4. reloads the server, verifies the login works, and
#   5. exports PG_PASSWORD so generate_env_file can write it into .env.
# Portable across Debian/Ubuntu, RHEL-family, and macOS (Homebrew).
provision_postgres() {
  local db_port="${DB_PORT:-5432}"

  if ${DRY_RUN}; then
    log_info "[DRY-RUN] Would provision PostgreSQL (set postgres password, pg_hba scram, write DB_* to .env)"
    PG_PASSWORD=""
    return 0
  fi

  if ! command -v psql >/dev/null 2>&1 && ! command -v pg_isready >/dev/null 2>&1; then
    log_warning "PostgreSQL tools not found — is the postgresql package installed?"
    PG_PASSWORD=""
    return 1
  fi

  # 1. Wait for the server to be reachable over the local TCP socket.
  log_info "Waiting for PostgreSQL to accept connections on 127.0.0.1:${db_port}..."
  local wait_loops=0
  ...
  # 2. Locate pg_hba.conf — from the running server's data directory (Linux
  #    /proc), falling back to well-known locations (Debian, RHEL, macOS).
  local pg_hba data_dir pg_pid
  pg_hba=""
  data_dir=""
  pg_pid="$(ss -tlnp 2>/dev/null | grep ":${db_port} " | grep -oE 'pid=[0-9]+' | head -1 | cut -d= -f2 || true)"
  if [ -n "${pg_pid}" ] && [ -r "/proc/${pg_pid}/cmdline" ]; then
    data_dir="$(tr '\0' ' ' < "/proc/${pg_pid}/cmdline" | grep -oE '\-D[[:space:]]+[^[:space:]]+' | head -1 | awk '{print $2}' || true)"
  fi
  if [ -n "${data_dir}" ] && [ -f "${data_dir}/pg_hba.conf" ]; then
    pg_hba="${data_dir}/pg_hba.conf"
  else
    pg_hba="$(find /etc/postgresql /var/lib/pgsql /var/lib/postgresql /opt/homebrew/var /usr/local/var -name pg_hba.conf 2>/dev/null | head -1)"
  fi

  # 3. Force SCRAM (password) auth for loopback TCP so the panel can connect.
  if [ -n "${pg_hba}" ]; then
    sed -i -E \
      -e 's#^(host\s+all\s+all\s+127\.0\.0\.1/32\s+)(md5|password|trust|peer|ident)(.*)$#\1scram-sha-256\3#' \
      -e 's#^(host\s+all\s+all\s+::1/128\s+)(md5|password|trust|peer|ident)(.*)$#\1scram-sha-256\3#' \
      "${pg_hba}" 2>/dev/null || true
    log_info "pg_hba.conf updated to use scram-sha-256 for loopback TCP (${pg_hba})"
  else
    log_warning "Could not locate pg_hba.conf — skipping auth hardening"
  fi

  # 4. Ensure the "postgres" superuser exists and set a fresh random password.
  local pg_pw
  pg_pw="$(openssl rand -base64 32 | tr -d '\n')"
  local pg_sql_file
  pg_sql_file="${TMPDIR:-/tmp}/nexuspanel_pg_${$}.sql"
  printf '%s\n' \
    "DO \$\$ BEGIN IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'postgres') THEN CREATE ROLE postgres LOGIN SUPERUSER; END IF; END \$\$;" \
    "ALTER ROLE postgres WITH LOGIN SUPERUSER PASSWORD '${pg_pw}';" \
    > "${pg_sql_file}"

  if [ "$(uname -s)" = "Darwin" ]; then
    # Homebrew: the current user is the initial superuser — connect over the socket.
    psql -p "${db_port}" -d postgres -f "${pg_sql_file}" 2>/dev/null || true
  elif command -v runuser >/dev/null 2>&1 && id postgres >/dev/null 2>&1; then
    runuser -u postgres -- psql -p "${db_port}" -d postgres -f "${pg_sql_file}" 2>/dev/null || true
  elif id postgres >/dev/null 2>&1; then
    su - postgres -c "psql -p '${db_port}' -d postgres -f '${pg_sql_file}'" 2>/dev/null || true
  else
    psql -p "${db_port}" -d postgres -f "${pg_sql_file}" 2>/dev/null || true
  fi
  rm -f "${pg_sql_file}"

  # 5. Reload so the pg_hba change takes effect.
  if [ "$(uname -s)" = "Darwin" ]; then
    brew services restart postgresql@16 2>/dev/null || brew services restart postgresql 2>/dev/null || true
  elif command -v systemctl >/dev/null 2>&1 && systemctl list-unit-files 2>/dev/null | grep -qE '^postgresql'; then
  ...
  # 6. Verify a TCP login with the new password actually works.
  sleep 1
  if PGPASSWORD="${pg_pw}" psql -h 127.0.0.1 -p "${db_port}" -U postgres -d postgres -tAc 'SELECT 1' >/dev/null 2>&1; then
    log_ok "PostgreSQL provisioned — postgres superuser login works over TCP"
    PG_PASSWORD="${pg_pw}"
  else
    log_warning "PostgreSQL password set, but the TCP login check failed (see pg_hba.conf / listen_addresses)"
    PG_PASSWORD="${pg_pw}"
  fi
}
