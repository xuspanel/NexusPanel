#!/usr/bin/env bash
# ============================================================================
# NexusPanel Installer v2.0 — CentOS Stream
# Platform: CentOS Stream 8/9
# Note: CentOS Linux 7/8 are EOL — use CentOS Stream, AlmaLinux, or Rocky
# ============================================================================
set -euo pipefail
IFS=$'\n\t'

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
if [ -f "${SCRIPT_DIR}/install-common.sh" ]; then
  source "${SCRIPT_DIR}/install-common.sh"
else
  source <(curl -sL "https://raw.githubusercontent.com/xuspanel/NexusPanel/main/nxApp/install-common.sh")
fi

OS_NAME="CentOS Stream"
VERSION_CODENAME=""

# ─── OS Detection ─────────────────────────────────────
detect_os() {
  if ! [ -f /etc/centos-release ] && ! [ -f /etc/redhat-release ]; then
    log_error "This script is for CentOS only"
    exit ${EXIT_GENERAL_ERROR}
  fi

  OS_NAME=$(head -1 /etc/centos-release 2>/dev/null || head -1 /etc/redhat-release)
  if ! echo "${OS_NAME}" | grep -qi "centos"; then
    log_warning "This appears to be ${OS_NAME}, not CentOS"
  fi

  # Extract version
  local ver
  ver=$(rpm -q --queryformat '%{VERSION}' centos-release 2>/dev/null || \
        rpm -q --queryformat '%{VERSION}' redhat-release 2>/dev/null || echo "0")
  VERSION_ID="${ver}"

  log_info "Detected: ${OS_NAME} ${VERSION_ID}"
}

# ─── Repositories ────────────────────────────────────
setup_repos() {
  log_info "Setting up repositories..."

  # EPEL
  if ! rpm -q epel-release >/dev/null 2>&1; then
    if [ "$(get_major_version)" -ge 9 ]; then
      run_cmd dnf install -y epel-release
      run_cmd dnf config-manager --set-enabled crb 2>/dev/null || true
    else
      run_cmd dnf install -y epel-release
      run_cmd dnf config-manager --set-enabled powertools 2>/dev/null || true
    fi
  fi

  # Node.js
  if [ ! -f /etc/yum.repos.d/nodesource-nodejs.repo ]; then
    if [ "$(get_major_version)" -ge 9 ]; then
      run_cmd bash <(curl -sL https://rpm.nodesource.com/setup_20.x)
    else
      # CentOS Stream 8
      run_cmd bash <(curl -sL https://rpm.nodesource.com/setup_18.x)
    fi
  fi

  log_success "Repositories configured"
}

# ─── Package Installation ──────────────────────────
install_packages() {
  log_info "Installing CentOS packages..."

  local pkgs=(
    nginx
    nodejs
    git
    policycoreutils-python-utils
    certbot
    python3-certbot-nginx
    bind-utils
    curl
    wget
    openssl
    fail2ban
    chrony
    logrotate
  )

  run_cmd dnf install -y "${pkgs[@]}"
  log_success "Packages installed"
}

# ─── SELinux ──────────────────────────────────────────
configure_selinux() {
  log_info "Configuring SELinux..."

  if ! command -v getenforce >/dev/null 2>&1; then
    log_info "SELinux not available — skipping"
    return 0
  fi

  if [ "$(getenforce)" = "Disabled" ]; then
    log_warning "SELinux is disabled — consider enabling"
    return 0
  fi

  # Allow Node.js to listen on privileged ports
  run_cmd semanage port -a -t http_port_t -p tcp "${PORT:-3443}" 2>/dev/null || \
    run_cmd semanage port -m -t http_port_t -p tcp "${PORT:-3443}" 2>/dev/null || true

  # Allow nginx to connect to backend
  run_cmd setsebool -P httpd_can_network_connect 1

  log_success "SELinux configured"
}

# ─── Main ─────────────────────────────────────────────
main() {
  show_banner
  setup_logging

  detect_os
  check_prerequisites "$@"
  setup_repos
  install_packages

  if ${INTERACTIVE}; then
    get_user_input
  fi

  if [ -n "${LICENSE_KEY:-}" ]; then
    validate_license "${LICENSE_KEY}" "${DOMAIN:-}" || exit ${EXIT_LICENSE_INVALID}
  fi

  install_app
  JWT_SECRET=$(openssl rand -hex 32)
  generate_env_file

  save_checkpoint "nginx"
  configure_nginx

  save_checkpoint "ssl"
  setup_ssl

  save_checkpoint "selinux"
  configure_selinux

  save_checkpoint "firewall"
  configure_firewall

  save_checkpoint "service"
  create_systemd_service
  start_service

  save_checkpoint "verify"
  sleep 2
  verify_installation || true

  clear_checkpoint
  show_summary
}

main "$@"
